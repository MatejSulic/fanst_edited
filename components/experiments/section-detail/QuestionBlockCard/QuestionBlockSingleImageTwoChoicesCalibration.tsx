...
const LockedQuestionBlockSingleImageTwoChoicesCalibration = ({
  question,
}: QuestionBlockSpecificCardSharedProps) => {
  return (
    <Box
...
const UnlockedQuestionBlockSingleImageTwoChoicesCalibration = ({
  question,
}: QuestionBlockSpecificCardSharedProps) => {
  ...
  const updateImagePublicId = (publicId: string) => {
    updateQuestionMutation.mutate({
      questionContent: { images: [publicId] },
    });
  };
      ...
      <Stack spacing={4}>
        ...
        >
          ...
          <TextField
            ...
            defaultValue={getImagesValuesSafe()[0]}
            {...register(
              `questions.${question._id.toString()}.content.images.0`
            )}        
          />
          {/* <Button
            size="small"
            variant="outlined"
            onClick={() => updateImagePublicId(question.content.images[0])}
          >
            Update image
          </Button> */}
        </Box>
...
const QuestionBlockSingleImageTwoChoicesCalibration = ({
  question,
  locked,
}: QuestionBlockCardSharedProps) => {
  return locked ? (
    <LockedQuestionBlockSingleImageTwoChoicesCalibration question={question} />
  ) : (
    <UnlockedQuestionBlockSingleImageTwoChoicesCalibration question={question} />
  );
};

export default QuestionBlockSingleImageTwoChoicesCalibration;
